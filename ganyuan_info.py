...
def main(ganyuan):
    ...
    # 没找到就返回“未找到…”
    zhi_ye = clean_wikitext(zhi_ye.group(1)).strip() if zhi_ye else "未找到职业"
    fen_zhi = clean_wikitext(fen_zhi.group(1)).strip() if fen_zhi else "未找到分支"
    xing_ji = str(f"{int(xing_ji.group(1).strip()) + 1}星") if xing_ji else "未找到稀有度"
    te_xing = clean_wikitext(te_xing.group(1)).strip() if te_xing else "未找到特性"
    te_xing_b = clean_wikitext(te_xing_b.group(1)).strip() if te_xing_b else "未找到特性备注"
