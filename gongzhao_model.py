...
class GongzhaoModel:
    ...
    async def get_public_recruitment_data(self):
        """异步获取所有可公开招募的干员数据。"""
    ...
    async def run(self):
        ...
        operators_data = await self.get_public_recruitment_data()
...
# 延迟初始化
gongzhao_model_instance = None

def initialize_gongzhao_model(config):
    global gongzhao_model_instance
    if gongzhao_model_instance is None:
        gongzhao_model_instance = GongzhaoModel(config)
    return gongzhao_model_instance